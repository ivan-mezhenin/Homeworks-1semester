...
int main(void) {
    ...
    scanf("%d", &length);
    printf("\n");
    int* unsortedArray = malloc(length * sizeof(int));
    fillingArrayOfRandowNumbers(unsortedArray, length);

    if (length < 50) {
        printf("Array Before sorting:\n");
        arrayOutput(unsortedArray, length);
    }
    printf("\n");

    clock_t timeStart = clock();
    countingSort(unsortedArray, length);
